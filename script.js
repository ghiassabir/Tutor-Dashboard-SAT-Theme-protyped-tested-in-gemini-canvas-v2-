...
document.addEventListener('DOMContentLoaded', function () {
    ...
    tutorTabs.forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            if (targetContentId === 'class-overview-content') loadClassOverviewData();
                ...
                else { replicatedStudentDashboardDiv.classList.add('hidden'); studentSelectionPrompt.classList.remove('hidden'); selectedStudentNameHeader.textContent = "";}
            }
        });
    });
    
    studentSelector.addEventListener('change', function() {
        if (this.value) { loadStudentDashboard(this.value); studentSelectionPrompt.classList.add('hidden'); }
        else { replicatedStudentDashboardDiv.classList.add('hidden'); studentSelectionPrompt.classList.remove('hidden'); selectedStudentNameHeader.textContent = ""; }
    });

    if (tutorTabs.length > 0) tutorTabs[0].click();
    ...
    document.querySelectorAll('.student-main-tab-button').forEach(tab => {
        tab.addEventListener('click', () => {
            ...
            tab.classList.add('active'); // Re-add active to current student tab
...
function initializeStudentOverviewCharts(student) { 
    ...
    const primaryChartColor = '#2a5266'; // SAT Hub Teal
    const barChartPrimaryBg = 'rgba(42, 82, 102, 0.8)'; // Darker Teal Translucent for bars
    const barChartSecondaryBg = 'rgba(108, 117, 125, 0.7)'; // Grey Translucent for comparison bars

    if(studentScoreTrendChartInstance) studentScoreTrendChartInstance.destroy();
    studentScoreTrendChartInstance = new Chart(document.getElementById('studentScoreTrendChart').getContext('2d'), { 
        type: 'line', data: { labels: student.cbPracticeTests.filter(t => t.total !== "-").map(t => t.name.replace("Official Practice ","").replace("Test ","T")), datasets: [{ label: 'Your Score', data: student.scoreHistory, borderColor: primaryChartColor, tension: 0.1 }] }, options: chartOptions });
    if(studentOverallSkillChartInstance) studentOverallSkillChartInstance.destroy();
    const studentSkillLabels = Object.values(student.cbSkills).flat().map(s => s.name.split(": ")[1] || s.name.split(" ")[0]); const studentSkillScores = Object.values(student.cbSkills).flat().map(s => s.score); const studentSkillClassAvgs = Object.values(student.cbSkills).flat().map(s => classAverages.cbSkills[s.key] || s.classAvg); 
    studentOverallSkillChartInstance = new Chart(document.getElementById('studentOverallSkillChart').getContext('2d'), { type: 'bar', data: { labels: studentSkillLabels.slice(0,8), datasets: [ { label: 'Your Accuracy', data: studentSkillScores.slice(0,8), backgroundColor: barChartPrimaryBg}, { label: 'Class Average', data: studentSkillClassAvgs.slice(0,8), backgroundColor: barChartSecondaryBg} ] }, options: { ...chartOptions, scales: { y: { beginAtZero: true, max: 100 }}} });
}

function loadClassOverviewData() { 
    const chartOptions = { responsive: true, maintainAspectRatio: true, plugins: { legend: { display: true, position: 'bottom' }}};
    const primaryChartColor = '#2a5266'; // SAT Hub Teal for charts
    const primaryChartBg = 'rgba(42, 82, 102, 0.8)'; // Darker Teal Translucent for bars
    const primaryChartRadarBg = 'rgba(42, 82, 102, 0.3)';
    ...
    classScoreDistributionChartInstance = new Chart(document.getElementById('classScoreDistributionChart').getContext('2d'), { type: 'bar', data: { labels: Object.keys(scoreCounts).map(bin => `${bin}-${parseInt(bin)+99}`).sort(), datasets: [{ label: '# of Students', data: Object.values(scoreCounts), backgroundColor: primaryChartBg }] }, options: { ...chartOptions, scales: { y: { beginAtZero: true, ticks: { stepSize: 1 } } } } }); 
    ...
    classSkillPerformanceChartInstance = new Chart(document.getElementById('classSkillPerformanceChart').getContext('2d'), { type: 'radar', data: { labels: skillKeysForChart.map(k=>k.split('_')[1].charAt(0).toUpperCase() + k.split('_')[1].slice(1) || k.split('_')[0]), datasets: [{ label: 'Class Avg Accuracy (%)', data: skillKeysForChart.map(key => classAverages.cbSkills[key] || 0), fill: true, backgroundColor: primaryChartRadarBg, borderColor: primaryChartColor, pointBackgroundColor: primaryChartColor }] }, options: { ...chartOptions, scales: { r: { beginAtZero: true, max: 100, suggestedMin: 40, pointLabels: { font: { size: 10 } } } } } }); 
...
function openModal(title, contentDetails) { 
    ...
    modalDonutChartInstance=new Chart(document.getElementById('modalDonutChart'),{type:'doughnut',data:{labels:['Correct','Incorrect','Unanswered'],datasets:[{data:[cor,inc,un],backgroundColor:['#28a745','#dc3545','#6c757d'],hoverOffset:4}]},options:{responsive:true,maintainAspectRatio:true,plugins:{legend:{position:'bottom'}},cutout:'50%'}}); 
    modalLineChartInstance=new Chart(document.getElementById('modalLineChart'),{type:'line',data:{labels:['W1','W2','W3','W4','W5'],datasets:[{label:'Your Score Trend',data:Array.from({length:5},()=>50+Math.random()*40),borderColor:'#2a5266',fill:false},{label:'Class Avg Trend',data:Array.from({length:5},()=>45+Math.random()*35),borderColor:'#6c757d',borderDash:[5,5],fill:false}]},options:{responsive:true,maintainAspectRatio:true,scales:{y:{beginAtZero:true,max:100}},plugins:{legend:{position:'bottom'}}}}); 
    modal.style.display="block"; 
}
function closeModal() { modal.style.display = "none"; if (modalDonutChartInstance) modalDonutChartInstance.destroy(); if (modalLineChartInstance) modalLineChartInstance.destroy(); }
